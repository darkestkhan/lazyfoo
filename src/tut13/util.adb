  ...
  begin
    ...
    case Transformation_Combo is
      when 0 =>
        ...
        GL.Scale  (Float (2.0), 2.0, 0.0);
        GL.Translate
          ( Float (Texture.Img_Width   (Loaded_Texture)) / (-2.0),
            Float (Texture.Img_Height  (Loaded_Texture)) / (-2.0),
            0.0
          );
      when 1 =>
        GL.Translate
  ...
  begin
    case Events.To_Character (Symbol) is
      ...
      when 'q'        =>
        Speed := 1.0;
        Angle := 0.0;
        Transformation_Combo := Transformation_Combo + 1;
        if Transformation_Combo > 4 then
