...
package body Util is
  ...
  ---------------------------------------------------------------------------
  -- Textures
  Checker_Board_Texture: Texture.Texture;
  ...
  is
    use type GL.UInt;
    -- Checkerboard pixels
    CB_Width  : constant GL.UInt := 128;
    CB_Height : constant GL.UInt := 128;
    CB_Pixel_Count: constant GL.UInt := CB_Width * CB_Height;
    Checker_Board: Texture.UInt_Array (0 .. Integer (CB_Pixel_Count - 1));

    Success: Boolean := False;
  begin
    -- Go through the pixels
    for K in Checker_Board'Range loop
      if (((K / 128) / 16) mod 2) = 1 xor (((K mod 128) / 16) mod 2) = 1 then
        Checker_Board (K) := 16#FF_FF_FF_FF#;
      else
        Checker_Board (K) := 16#FF_00_00_FF#;
      end if;
    end loop;

    -- Load texture
    Texture.Load_From_Pixels_32
      ( Checker_Board_Texture, Checker_Board'Address,
        CB_Width, CB_Height, Success
      );
    if not Success then
      TIO.Put_Line (TIO.Standard_Error, "Unable to load checkerboard texture");
      return False;
    else
      return True;
    end if;
  ...
  is
    ...
    X: constant Float :=
        Float (GL.UInt (Screen_Width) - Texture.Width (Checker_Board_Texture))
      / Float (2.0);
    Y: constant Float :=
        Float (GL.Uint (Screen_Height) - Texture.Height (Checker_Board_Texture))
      / Float (2.0);
  begin
    -- Clear color buffer
    ...
    Texture.Render (Checker_Board_Texture, X, Y);
