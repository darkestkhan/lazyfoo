...
package body Texture is
  ...
  Default_Texture_Wrap: constant GL.Enum := GL.GL_REPEAT;
  ...
  procedure Init_VBO (This: in out Texture)
  is
    use type GL.UInt;

    -- Vertex data
    vData: Vertex_Data_2D_Array (0 .. 3);
    iData: UInt_Array (0 .. 3);
  begin
    if (This.ID /= 0 and This.VBO_ID = 0) then
      -- Set rendering indices
      for K in iData'range loop
        iData (K) := GL.UInt (K);
      end loop;

      -- Create VBO
      GL.Gen_Buffers (1, This.VBO_ID'Address);
      GL.Bind_Buffer (GL.GL_ARRAY_BUFFER, This.VBO_ID);
      GL.Buffer_Data
        ( GL.GL_ARRAY_BUFFER,
          vData'Length * (Vertex_Data_2D'Size / 8),
          vData'Address,
          GL.GL_DYNAMIC_DRAW
        );

      -- Create IBO
      GL.Gen_Buffers (1, This.IBO_ID'Address);
      GL.Bind_Buffer (GL.GL_ELEMENT_ARRAY_BUFFER, This.IBO_ID);
      GL.Buffer_Data
        ( GL.GL_ELEMENT_ARRAY_BUFFER,
          iData'Length * (GL.UInt'Size / 8),
          iData'Address,
          GL.GL_DYNAMIC_DRAW
        );
      ...
      GL.Bind_Buffer (GL.GL_ELEMENT_ARRAY_BUFFER, 0);
    end if;
  end Init_VBO;
  ...
  -- Side:
  --    None
  procedure Free_VBO (This: in out Texture)
  is
  begin
    GL.Delete_Buffers (1, This.VBO_ID'Address);
    GL.Delete_Buffers (1, This.IBO_ID'Address);
  end Free_VBO;
  ...
  procedure Free_Texture (This: in out Texture)
  is
    use type GL.UInt;
  begin
    if This.ID /= 0 then
  ...
  begin
    ...
    if ID (This) /= 0 then
      ...
      begin
        ...
        -- Texture coordinates
        vData (0).Tex_Coord.S := Tex_Left;
        vData (0).Tex_Coord.T := Tex_Top;

        vData (1).Tex_Coord.S := Tex_Right;
        vData (1).Tex_Coord.T := Tex_Top;

        vData (2).Tex_Coord.S := Tex_Right;
        vData (2).Tex_Coord.T := Tex_Bottom;

        vData (3).Tex_Coord.S := Tex_Left;
        vData (3).Tex_Coord.T := Tex_Bottom;

        -- Vertex positions
        vData (0).Position.X  := 0.0;
        vData (0).Position.Y  := 0.0;

        vData (1).Position.X  := Quad_Width;
        vData (1).Position.Y  := 0.0;

        vData (2).Position.X  := Quad_Width;
        vData (2).Position.Y  := Quad_Height;

        vData (3).Position.X  := 0.0;
        vData (3).Position.Y  := Quad_Height;
        ...
        GL.Enable_Client_State (GL.GL_TEXTURE_COORD_ARRAY);
        begin
          -- Bind vertex buffer
          GL.Bind_Buffer (GL.GL_ARRAY_BUFFER, This.VBO_ID);
          ...
          GL.Tex_Coord_Pointer (2, GL.GL_FLOAT, Vertex_Data_2D'Size / 8, 8);

          -- Set vertex data
          GL.Vertex_Pointer (2, GL.GL_FLOAT, Vertex_Data_2D'Size / 8, 0);
        ...
        GL.Disable_Client_State (GL.GL_TEXTURE_COORD_ARRAY);
