...
private
  ...
  type Texture is
  record
    ID        : GL.UInt   := 0;  -- Texture ID/name
    -- Texture dimensions
    Width     : GL.UInt   := 0;  -- Texture width
    Height    : GL.UInt   := 0;  -- Texture height
    -- Unpadded image dimensions
    Img_Width : GL.UInt   := 0;  -- Image width
    Img_Height: GL.UInt   := 0;  -- Image height
    -- Current pixels
  ...
  type Tex_Coord_2D is
  record
    S: Float := 0.0;
    T: Float := 0.0;
  end record;

  for Tex_Coord_2D use
  record
    S at 0 * Word range 0 .. 31;
    T at 1 * Word range 0 .. 31;
  end record;

  for Tex_Coord_2D'Size use 64;

  ---------------------------------------------------------------------------

  type Vertex_Pos_2D is
  record
    X: Float := 0.0;
    Y: Float := 0.0;
  end record;

  for Vertex_Pos_2D use
  record
    X at 0 * Word range 0 .. 31;
    Y at 1 * Word range 0 .. 31;
  end record;

  for Vertex_Pos_2D'Size use 64;

  ---------------------------------------------------------------------------

  type Vertex_Data_2D is
  record
    Position  : Vertex_Pos_2D;
    Tex_Coord : Tex_Coord_2D;
  end record;

  for Vertex_Data_2D use
  record
    Position  at 0 * Word range 0 .. 63;
    Tex_Coord at 2 * Word range 0 .. 63;
  end record;

  for Vertex_Data_2D'Size use 128;

  type Vertex_Data_2D_Array is array (Natural range <>) of Vertex_Data_2D;
